...
void setup(){
        ...
        startTimer(TC1, 0, TC3_IRQn, 5760); //TC1 channel 0, the IRQ for that channel and the desired frequency
...
//  pPrint++;
  ...
  //envoi des echantillons 16 bits en 2X8bits pour envoi sur port série(limité à 8bits)
//  lsb=(char)(*pPrint);                
//  Serial2.write(lsb);
  //Serial.println(lsb);
//  msb=(char)(*pPrint>>8);
...
void echantillonnage(){
  ...
  *pRead=/*Const*/ analogRead(sonInPin);                      
    ...
    if (int(pRead)==Max+int(buffer)){            
      ...
      //si la variation ne dépasse pas un minimum = pas de son
      //version 8bits
      //if (*pRead>125 && *pRead<129){
      //*pRead=127;

      //version 16bits
      if (*pRead>2040 && *pRead<2054){
      *pRead=2047;
      //Const++;
      }
     pRead++; 
